...
**Then** `GET /api/admin/health` (proxied to NestJS, protected by AdminGuard) returns the system health dashboard with: estimated CCU, active group count, storage usage %, email quota remaining today

**Given** I navigate to `/admin/users`
**When** the page loads
**Then** `GET /api/admin/users` (proxied to NestJS, protected by AdminGuard) returns all user profiles with display name, email, join date, group count, and active status
...
**Then** `POST /api/admin/users/:userId/warn` (proxied to NestJS, protected by AdminGuard) logs the warning via Prisma
...
**Then** `POST /api/admin/users/:userId/ban` (proxied to NestJS, protected by AdminGuard) executes
...
**When** the page loads
**Then** `GET /api/admin/content/flagged` (proxied to NestJS, protected by AdminGuard) returns all soft-deleted lessons (`is_deleted = true`) and any editor-flagged content
**And** I can permanently delete or restore any soft-deleted content via `DELETE /api/admin/content/:id` or `PATCH /api/admin/content/:id/restore`

**Given** I navigate to `/admin/groups`
**When** the page loads
**Then** `GET /api/admin/groups` (proxied to NestJS, protected by AdminGuard) returns all groups with name, member count, lesson count, and last activity date
...
**When** the page loads
**Then** `GET /api/admin/analytics` (proxied to NestJS, protected by AdminGuard) returns key KPIs: total active users, MoM retention rate, 7-day streak retention %, average contributor-to-learner ratio per group, peer review completion rate
