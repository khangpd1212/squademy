import { allowedAvatarMimeTypes, maxAvatarSizeBytes } from "@/app/(dashboard)/settings/profile-schema";
import { createClient } from "@/lib/supabase/server";
import { NextResponse } from "next/server";

function isAllowedAvatarType(type: string) {
  return allowedAvatarMimeTypes.includes(type as (typeof allowedAvatarMimeTypes)[number]);
}
...
export async function POST(request: Request) {
  ...
  if (!isAllowedAvatarType(fileEntry.type)) {
    return NextResponse.json({ message: "Avatar must be a JPG or PNG image." }, { status: 400 });
  }

  if (fileEntry.size === 0) {
    return NextResponse.json({ message: "Avatar file is empty." }, { status: 400 });
  }
